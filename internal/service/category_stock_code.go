...
// 插入记录
func (cscs *categoryStockCodeService) Insert(ctx context.Context, entity *model.CategoryStockCode) (err error) {
	_, err = model.DB(ctx, model.CategoryStockCodeTableInfo.Table()).Insert(entity)
	return
}

// 删除所有数据
func (cscs *categoryStockCodeService) DeleteAll(ctx context.Context) (err error) {
	_, err = model.DB(ctx, model.CategoryStockCodeTableInfo.Table()).Delete()
	return
}

// 删除指定类型的数据
func (cscs *categoryStockCodeService) DeleteByType(ctx context.Context, typeName string) (err error) {
